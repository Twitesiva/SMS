 ...
 import "./LibraryPortalShell.css";

 const SIDEBAR_SCROLL_KEY = "library-shell-sidebar-scroll";
 const isUuid = (value = "") =>
 ...
 export default function LibraryPortalShell({
   children,
   onSignOut,
   navGroups,
   ...
   useEffect(() => {
     ...
     if (user?.id && isUuid(user.id)) {
       ...
       logActivity(supabase, {
         ...
         action: "VIEW",
         page: pageLabel,
         description: `${(user.role || "User").charAt(0).toUpperCase() +
           (user.role || "user").slice(1).toLowerCase()} viewed page ${pageLabel}`,
       });
     }
